...
chown www-data:www-data "$data_dir/gpg"
...
chown www-data:www-data "$data_dir/jwt"
rm -rf "$passbolt_config/jwt"
ln -s "$data_dir/jwt" "$passbolt_config/jwt"

# continue with this file: https://github.com/passbolt/passbolt_docker/blob/master/debian/bin/docker-entrypoint.sh
